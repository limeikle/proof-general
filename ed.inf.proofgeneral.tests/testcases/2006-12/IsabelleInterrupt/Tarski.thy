(*  Title:      HOL/ex/Tarski.thy
    ID:         $Id: Tarski.thy,v 1.2 2006/12/19 15:22:46 da Exp $
    Author:     Florian Kamm�ller, Cambridge University Computer Laboratory
*)
...
apply (simp add: isLub_upper)
-- {* @{text "\<forall>z:interval r a b. (\<forall>y:S. (y, z) \<in> induced (interval r a b) r --> (if S = {} then a else L, z) \<in> induced (interval r a b) r"} *}
apply (rule ballI)
apply (rule impI)
apply (case_tac "S = {}")
-- {* @{text "S = {}"} *}
...
lemma (in Tarski) intY1_f_closed: "x \<in> intY1 ==> f x \<in> intY1"
