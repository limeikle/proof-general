...
Note on parsing this:
   - Takes a good number of seconds (10ish) in HOL for me
   - Then Eclipse goes a bit mad: seems almost even to be infinite
     loop. 
 *)
...
consts I_intterm :: "int list ⇒ intterm ⇒ int"
primrec 
...
"qinterp ats (Le it1 it2) = (I_intterm ats it1 ≤ I_intterm ats it2)"
...
consts lift_bin:: "('a ⇒ 'a ⇒ 'b) × 'a option × 'a option ⇒ 'b option"
recdef lift_bin "measure (λ(c,a,b). size a)"
...
consts lift_un:: "('a ⇒ 'b) ⇒ 'a option ⇒ 'b option"
primrec
"lift_un c None = None"
"lift_un c (Some p) = Some (c p)"

consts lift_qe:: "('a ⇒ 'b option) ⇒ 'a option ⇒ 'b option"
primrec
...
lemma qelim_qfree: 
  assumes qeqf: "(⋀ q q'. [|isqfree q ; qe q = Some q'|] ==>  isqfree q')"
  shows qff:"⋀ p'. qelim (qe, p) = Some p' ==> isqfree p'"
  using qeqf
proof (induct p)
  case (Lt a b)
...
next  
  case (NOT p)
  from "NOT.prems" have "∃ p1. qelim(qe,p) = Some p1"
    by  (cases "qelim(qe,p)") simp_all
  then obtain "p1" where p1_def: "qelim(qe,p) = Some p1" by blast
  from "NOT.prems" have "⋀q q'. [|isqfree q; qe q = Some q'|] ==> isqfree q'" 
    by blast
...
next  
  case (And p q) 
  from "And.prems" have p1q1: "(∃p1. qelim(qe,p) = Some p1) ∧ 
    (∃q1. qelim(qe,q) = Some q1)" using lift_bin_Some[where c="And"] by simp
...
next  
  case (Or p q)
  from "Or.prems" have p1q1: "(∃p1. qelim(qe,p) = Some p1) ∧ 
    (∃q1. qelim(qe,q) = Some q1)" using lift_bin_Some[where c="Or"] by simp
  from p1q1 obtain "p1" and "q1" 
    where p1_def: "qelim(qe,p) = Some p1" 
    and q1_def: "qelim(qe,q) = Some q1" by blast
  from prems have qf1:"isqfree p1"
...
next  
  ...
  from "Imp.prems" have p1q1: "(∃p1. qelim(qe,p) = Some p1) ∧ 
    (∃q1. qelim(qe,q) = Some q1)" using lift_bin_Some[where c="Imp"] by simp
  from p1q1 obtain "p1" and "q1" 
    where p1_def: "qelim(qe,p) = Some p1" 
    and q1_def: "qelim(qe,q) = Some q1" by blast
  from prems have qf1:"isqfree p1"
...
next  
  ...
  from "Equ.prems" have p1q1: "(∃p1. qelim(qe,p) = Some p1) ∧ 
    (∃q1. qelim(qe,q) = Some q1)" using lift_bin_Some[where c="Equ"] by simp
  from p1q1 obtain "p1" and "q1" 
    where p1_def: "qelim(qe,p) = Some p1" 
    and q1_def: "qelim(qe,q) = Some q1" by blast
  from prems have qf1:"isqfree p1"
...
next 
  case (QAll p) 
  from "QAll.prems"
  have "∃ p1. lift_qe qe (lift_un NOT (qelim (qe ,p))) = Some p1" 
  ...
  then have "∃ p2. lift_un NOT (qelim (qe ,p)) = Some p2"
    by (cases "qelim (qe ,p)") simp_all
  then obtain "p2" 
    where p2_def:"lift_un NOT (qelim (qe ,p)) = Some p2" by blast
  then have "∃ p3. qelim(qe,p) = Some p3" by (cases "qelim(qe,p)") simp_all
...
proof (induct p)
  case (NOT f)  
  from "NOT.prems" have "∃f'. ?Qe f f' " by (cases "qelim(qe,f)") simp_all
...
next
  ...
  from "And.prems" have f1g1: "(∃f1. qelim(qe,f) = Some f1) ∧ 
    (∃g1. qelim(qe,g) = Some g1)" using lift_bin_Some[where c="And"] by simp
...
next
  ...
  from "Or.prems" have f1g1: "(∃f1. qelim(qe,f) = Some f1) ∧ 
    (∃g1. qelim(qe,g) = Some g1)" using lift_bin_Some[where c="Or"] by simp
  from f1g1 obtain "f1" and "g1" 
    where f1_def: "qelim(qe, f) = Some f1" 
    and g1_def: "qelim(qe,g) = Some g1" by blast
  from prems f1_def have feqf1: "?F ats f = ?F ats  f1" by blast
...
next
  ...
  from "Imp.prems" have f1g1: "(∃f1. qelim(qe,f) = Some f1) ∧ 
    (∃g1. qelim(qe,g) = Some g1)" using lift_bin_Some[where c="Imp"] by simp
  from f1g1 obtain "f1" and "g1" 
    where f1_def: "qelim(qe, f) = Some f1" 
    and g1_def: "qelim(qe,g) = Some g1" by blast
  from prems f1_def have feqf1: "?F ats f = ?F ats f1" by blast
...
next
  ...
  from "Equ.prems" have f1g1: "(∃f1. qelim(qe,f) = Some f1) ∧ 
    (∃g1. qelim(qe,g) = Some g1)" using lift_bin_Some[where c="Equ"] by simp
  from f1g1 obtain "f1" and "g1" 
    where f1_def: "qelim(qe, f) = Some f1" 
    and g1_def: "qelim(qe,g) = Some g1" by blast
  from prems f1_def have feqf1: "?F ats f = ?F ats f1" by blast
...
next
  ...
  then  have "∃ f'. qe f1 = Some f'" using f1_def by simp
...
next
  case (QAll f)
  from "QAll.prems"
  have "∃ f0. lift_un NOT (lift_qe qe (lift_un NOT (qelim (qe ,f)))) = 
    Some f0"
  ...
  then obtain "f0" 
    ...
    Some f0" by blast
  then have "∃ f1. lift_qe qe (lift_un NOT (qelim (qe ,f))) = Some f1" 
  ...
  have "?F ats (QAll f) = (∀x. ?F (x#ats) f)" by simp
  also have "… = (¬ (∃ x. ?F (x#ats) (NOT f)))" by simp
  also have "… = (¬ (∃ x. ?F (x#ats) (NOT (NOT f2))))" using feqf2
    by auto
  also have "… = (¬ (∃ x. ?F (x#ats) f2))" by simp
  also have "… = (¬ (?F ats f1))" using exf2eqf1 by simp
...
lemma nnf_corr: "isqfree p ==> qinterp ats p = qinterp ats (nnf p)"
...
lemma nnf_isnnf : "isqfree p ==> isnnf (nnf p)"
by (induct p rule: nnf.induct, auto)

lemma nnf_isqfree: "isnnf p ==> isqfree p"
by (induct p rule: isnnf.induct) auto

(* nnf preserves quantifier freeness *)
lemma nnf_qfree: "isqfree p ==> isqfree(nnf p)"
...
lemma islininttermc0r: "islinintterm (Add (Mult (Cst c) (Var n)) r) ==> (c ≠ 0 ∧ islinintterm r)"
...
lemma nth_pos: "0 < n --> (x#xs) ! n = (y#xs) ! n"
...
using advdd  
proof-
  from advdd  have "∀x.∀k. (((a::int) dvd (x + t)) = (a dvd
 (x+k*d + t)))" by (rule dvd_modd_pinf)
  then show ?thesis by simp
qed

(* lin_ad adds two linear terms*)
consts lin_add :: "intterm × intterm ⇒ intterm"
recdef lin_add "measure (λ(x,y). ((size x) + (size y)))"
"lin_add (Add (Mult (Cst c1) (Var n1)) (r1),Add (Mult (Cst c2) (Var n2)) (r2)) =
  ...
  else if n1 ≤ n2 then (Add (Mult (Cst c1) (Var n1)) (lin_add (r1,Add (Mult (Cst c2) (Var n2)) (r2)))) 
...
lemma lin_add_corrh: "⋀ n01 n02. [| islintn (n01,a) ; islintn (n02,b)|] 
  ==> I_intterm ats (lin_add(a,b)) = I_intterm ats (Add a b)"
proof(induct a b rule: lin_add.induct)
  case (58 i n r j m s) 
  have "(n = m ∧ i+j = 0) ∨ (n = m ∧ i+j ≠ 0) ∨ n < m ∨ m < n " by arith
  moreover
  {assume "n=m∧i+j=0" hence ?case using prems by (auto simp add: sym[OF zadd_zmult_distrib]) }
  moreover
  {assume "n=m∧i+j≠0" hence ?case using prems by (auto simp add: Let_def zadd_zmult_distrib)}
  moreover
...
lemma lin_add_lint: "⋀ n0 n01 n02. [| islintn (n01,a) ; islintn (n02,b); n0 ≤  min n01 n02 |] 
  ==> islintn (n0, lin_add (a,b))"
proof (induct a b rule: lin_add.induct)
  case (58 i n r j m s)
  have "(n =m ∧ i + j = 0) ∨ (n = m ∧ i+j ≠ 0) ∨ n <m ∨ m < n" by arith
  moreover 
...
consts lin_mul :: "int × intterm ⇒ intterm"
recdef lin_mul "measure (λ(c,t). size t)"
...
"linearize (Neg i) = lift_un lin_neg (linearize i)"
 "linearize (Add i j) = lift_bin(λ x. λ y. lin_add(x,y), linearize i, linearize j)"
"linearize (Sub i j) = 
  lift_bin(λ x. λ y. lin_add(x,lin_neg y), linearize i, linearize j)"
"linearize (Mult i j) = 
  (case linearize i of
  None ⇒ None
  | Some li ⇒ (case li of 
     Cst b ⇒ (case linearize j of
      None ⇒ None
     | (Some lj) ⇒ Some (lin_mul(b,lj)))
  | _ ⇒ (case linearize j of
      None ⇒ None
    | (Some lj) ⇒ (case lj of 
        Cst b ⇒ Some (lin_mul (b,li))
      | _ ⇒ None))))"
...
next
  case (6 i j)show ?case 
	...
	∨ ((∃ li. linearize i = Some li ∧ ¬ (∃ bi. li = Cst bi)) ∧ (∃ lj. linearize j = Some lj ∧ ¬ (∃ bj. lj = Cst bj)))" by auto 
    moreover 
    {
      assume nlini: "linearize i = None"
	...
	and ljnc: "∃ lj. linearize j = Some lj ∧ ¬ (∃ bj. lj = Cst bj)"
      from linc obtain "li" where "linearize i = Some li ∧ ¬ (∃ bi. li = Cst bi)" by blast
      moreover 
      from ljnc obtain "lj" where "linearize j = Some lj ∧ ¬ (∃ bj. lj = Cst bj)" by blast
...
lemma linearize_linear: "⋀ t'. linearize t = Some t' ==> islinintterm t'"
proof-
  fix t'
...
next
  case (6 i j)show ?case 
    ...
    { assume linc: "∃ li. linearize i = Some li ∧ ¬ (∃ bi. li = Cst bi)"
	and ljnc: "∃ lj. linearize j = Some lj ∧ ¬ (∃ bj. lj = Cst bj)"
      from linc obtain "li" where "∃ li. linearize i = Some li ∧ ¬ (∃ bi. li = Cst bi)" by blast
      moreover 
      from ljnc obtain "lj" where "∃ lj. linearize j = Some lj ∧ ¬ (∃ bj. lj = Cst bj)" by blast
      ultimately have "linearize (Mult i j) = None"
	apply simp
...
lemma linearize_corr: "⋀ t'. linearize t = Some t' ==> I_intterm ats t = I_intterm ats t' "
proof-
  fix t'
  assume lint: "linearize t = Some t'"
...
consts linform :: "QF ⇒ QF option"
primrec
"linform (Le it1 it2) =  
  lift_bin(λx. λy. Le (lin_add(x,lin_neg y)) (Cst 0),linearize it1, linearize it2)"
"linform (Eq it1 it2) =  
  lift_bin(λx. λy. Eq (lin_add(x,lin_neg y)) (Cst 0),linearize it1, linearize it2)"
"linform (Divides d t) =  
  (case linearize d of
    None ⇒ None
   | Some ld ⇒ (case ld of
          Cst b ⇒ 
               (if (b=0) then None
                ...
                (case linearize t of 
                 None ⇒ None
               | Some lt ⇒ Some (Divides ld lt)))
         | _ ⇒ None))"
"linform  T = Some T"
"linform  F = Some F"
"linform (NOT p) = lift_un NOT (linform p)"
"linform (And p q)= lift_bin(λf. λg. And f g, linform p, linform q)"
"linform (Or p q) = lift_bin(λf. λg. Or f g, linform p, linform q)"

(* linearity of formulae *)
consts islinform :: "QF ⇒ bool"
recdef islinform "measure size"
"islinform (Le it (Cst i)) = (i=0 ∧ islinintterm it )"
"islinform (Eq it (Cst i)) = (i=0 ∧ islinintterm it)"
"islinform (Divides (Cst d) t) = (d ≠ 0 ∧ islinintterm t)"
"islinform  T = True"
"islinform  F = True"
"islinform (NOT (Divides (Cst d) t)) = (d ≠ 0 ∧ islinintterm t)"
"islinform (NOT (Eq it (Cst i))) = (i=0 ∧ islinintterm it)"
"islinform (And p q)= ((islinform p) ∧ (islinform q))"
"islinform (Or p q) = ((islinform p) ∧ (islinform q))"
"islinform p = False"

(* linform preserves nnf, if successful *)
lemma linform_nnf: 
  assumes nnfp: "isnnf p" 
  shows "⋀ p'. [|linform p = Some p'|] ==> isnnf p'"
using nnfp
...
lemma linform_corr: "⋀ lp. [| isnnf p ; linform p = Some lp |] ==> 
...
lemma linform_lin: "⋀ lp. [| isnnf p ; linform p = Some lp|] ==> islinform lp"
proof (induct p rule: linform.induct)
   case (Le x y)
  have "((∃ lx. linearize x = Some lx) ∧ (∃ ly. linearize y = Some ly)) ∨ 
    (linearize x = None) ∨ (linearize y = None) " by clarsimp
  moreover 
  {
    assume linx: "∃ lx. linearize x = Some lx"
      and liny: "∃ ly. linearize y = Some ly"
...
lemma linform_isnnf: "islinform p ==> isnnf p"
by (induct p rule: islinform.induct) auto

lemma linform_isqfree: "islinform p ==> isqfree p"
using linform_isnnf nnf_isqfree by simp

lemma linform_qfree: "⋀ p'. [| isnnf p ; linform p = Some p'|] ==> isqfree p'"
using linform_isqfree linform_lin 
by simp

(* Definitions and lemmas about gcd and lcm *)
definition
  lcm :: "nat × nat ⇒ nat" where
  "lcm = (λ(m,n). m*n div gcd(m,n))"

definition
  ilcm :: "int ⇒ int ⇒ int" where
...
lemma divideallc_mono: "⋀ c. [| divideallc(c,p) ; c dvd d|] ==> divideallc (d,p)"
...
recdef adjustcoeff "measure (λ(l,p). size p)"
...
"isunified (NOT(Divides (Cst d) (Add (Mult (Cst i) (Var 0)) r))) = 
  ((abs i) = 1 ∧ (islinform(NOT(Divides (Cst d) (Add (Mult (Cst i) (Var 0)) r)))))"
"isunified (And p q) = (isunified p ∧ isunified q)"
"isunified (Or p q) = (isunified p ∧ isunified q)"
"isunified p = islinform p"

lemma unified_islinform: "isunified p ==> islinform p"
...
lemma zmult_zle_mono: "(i::int) ≤ j ==> 0 ≤ k ==> k * i ≤ k * j"
...
lemma zmult_zle_mono_eq:
  assumes kpos: "0 < k"
  shows "((i::int) ≤ j) = (k*i ≤ k*j)" (is "?P = ?Q")
proof
  assume P: ?P
  from kpos have kge0: "0 ≤ k" by simp
  show ?Q
    by (rule zmult_zle_mono[OF P kge0])
next 
  assume ?Q
  then have "k*i - k*j ≤ 0" by simp
  then have le1: "k*(i-j) ≤ k*0"
...
proof-
  from lpos ipos have ilel: "i≤l" by (simp add: zdvd_imp_le [OF dvd lpos])
  from ipos have inz: "i ≠ 0" by simp
  have "i div i≤ l div i"
    by (simp add: zdiv_mono1[OF ilel ipos])
  then have ldivipos:"0 < l div i" 
    by (simp add: zdiv_self[OF inz])
  
  from dvd have "∃i'. i*i' = l" by (auto simp add: dvd_def)
  ...
  also
  have "(l div i * (i * x + r) ≤ l div i * 0) = ((l div i * i) * x + ((l div i)*r) ≤ 0)"
    by (simp add: mult_ac)
  also have "((l div i * i) * x + ((l div i)*r) ≤ 0) = (l*x + ((l div i)*r) ≤ 0)"
...
proof-
  ...
  from lpos ineg have milel: "-i≤l" by (simp add: zdvd_imp_le [OF midvdl lpos])
  from ineg have inz: "i ≠ 0" by simp
  have "l div i≤ -i div i"
  ...
  also
  have "(- (l div i) * (i * x + r) ≤ - (l div i) * 0) = (-((l div i) * i) * x ≤ (l div i)*r)"
    by (simp add: mult_ac)
  also have " (-((l div i) * i) * x ≤ (l div i)*r) = (- (l*x) ≤ (l div i)*r)"
    using sym[OF ii'eql] inz
    by (simp add: zmult_ac)
  finally  
  show ?thesis
...
proof-
  ...
  moreover
  {
    ...
    from bpos aneg have maleb: "-a≤b" by (simp add: zdvd_imp_le [OF midvdb bpos])
...
lemma adjustcoeff_unified: 
  ...
  proof(induct l p rule: adjustcoeff.induct,simp_all add: lin_mul_lintn islinintterm_eq_islint islint_def)
    ...
    moreover {
      ...
      have clel: "c ≤ l" by (rule zdvd_imp_le[OF cdvdl lp])
      have "c div c ≤  l div c" by (rule zdiv_mono1[OF clel cpos])
    ...
    ultimately  show ?case by blast
  next
    case (goal2 l p)    from prems have "c >0 ∨ c < 0" by auto
    moreover {
      assume cpos: "c > 0 "
      from prems have lp: "l > 0" by simp
      from prems have cdvdl: "c dvd l" by simp
...
lemma unified_isqfree: "isunified p==> isqfree p"
...
(* divlcm computes δ = lcm d , where d | x +t occurs in p *)
...
lemma alldivide_mono: "⋀ d'. [| alldivide (d,p) ; d dvd d'|] ==> alldivide (d',p)"
...
lemma nz_le: "(x::int) > 0 ==> x ≠ 0" by auto
(* divlcm is correct *)
lemma divlcm_corr:
  assumes 
  linp: "islinform p"
...
(* minusinf p and p are the same for minusinfity … *)
lemma minusinf_eq: 
  assumes unifp: "isunified p" 
  shows "∃ z. ∀ x. x < z --> (qinterp (x#ats) p = qinterp (x#ats) (minusinf p))"
...
proof (induct p rule: minusinf.induct)
  ...
  moreover 
  {
	...
	show ?thesis
      ...
      proof-
	fix x
	show "x < I_intterm (a # ats) r --> ¬ - x + I_intterm (x # ats) r ≤ 0"
...
next
  ...
  from prems obtain "zf" where 
    zf:"∀x<zf. qinterp (x # ats) f = qinterp (x # ats) (minusinf f)" by auto
  from prems obtain "zg" where 
    zg:"∀x<zg. qinterp (x # ats) g = qinterp (x # ats) (minusinf g)" by auto
...
lemma minusinf_repeats2:
  assumes alldvd: "alldivide (d,p)"
  and unity: "isunified p"
  shows "∀ x k. (qinterp (x#ats) (minusinf p) = qinterp ((x - k*d)#ats) (minusinf p))" 
  (is "∀ x k. ?P x = ?P (x - k*d)")
...
proof
  ...
  {
    assume "∃ j∈ {1 .. ?d}. ?P j"
    then show "∃ x. ?P x" using dpos  by auto
  next
    assume "∃ x. ?P x"
    then obtain "x" where P: "?P x" by blast
    have modd: "∀x k. ?P x = ?P (x - k*?d)"
      by (rule minusinf_repeats2[OF alldvd unifp])
    ...
    proof (cases)
      ...
      moreover have "?d ∈ {1 .. ?d}" using dpos 
	by (simp add:atLeastAtMost_iff)
      ultimately show "∃ j∈ {1 .. ?d}. ?P j" ..
    next 
      assume not0: "x mod ?d ≠ 0"
      have "?P(x mod ?d)" using dpos P Pmod by(simp add:pos_mod_sign pos_mod_bound)
      moreover have "x mod ?d : {1 .. ?d}"
      proof -
	have "0 ≤ x mod ?d" by(rule pos_mod_sign[OF dpos])
...
lemma bset_lin:
  assumes unifp: "isunified p"
  shows "∀ b ∈ set (bset p). islinintterm b"
using unifp unified_islinform[OF unifp]
...
lemma bset_disj_repeat:
  ...
  and dpos: "0 < d"
  and nob: "(qinterp (x#ats) q) ∧ ¬(∃j∈ {1 .. d}. ∃ b ∈ set (bset p). (qinterp (((I_intterm (a#ats) b) + j)#ats) q)) ∧(qinterp (x#ats) p)" 
...
proof (induct p rule: islinform.induct,auto)
  ...
  { assume "∃ i n r. t = Add (Mult (Cst i) (Var n) ) r"
	...
	proof(auto simp add: intterm_novar0[OF lininr, where x="x - d" and y="x"], cases)
	  assume "- x + d +  ?I x r ≤ 0"
	  then show "- x + d + ?I x r ≤ 0" .
	next 
	  assume np: "¬ - x + d +  ?I x r ≤ 0"
	  then have ltd:"x - ?I x r ≤ d - 1" by simp 
	  from prems have "-x + ?I x r ≤ 0" by simp
	  then have ge0: "x - ?I x r \<ge> 0" 
	    by simp
	  from ltd ge0 have "x - ?I x r = 0 ∨ (1 ≤ x - ?I x r ∧ x - ?I x r ≤ d - 1) " by arith
	  moreover
	  {
	    assume "x - ?I x r = 0"
	    ...
	    from prems 
	    have "∀j∈{1..d}. ¬ ?Q (?I a r + -1 + j)"
	      using linr by (auto simp add: lin_add_corr)
	    moreover from dpos have "1 ∈ {1..d}" by simp
	    ultimately have " ¬ ?Q (?I a r + -1 + 1)" by blast
	    with dpos linr have "¬ ?Q (?I x r)"
	      by (simp add: intterm_novar0[OF lininr, where x="x" and y="a"] lin_add_corr)
	    with qr have "- x + d + ?I x r ≤ 0" by simp
	  }
	  moreover
	  {
	    assume gt0: "1 ≤ x - ?I x r ∧ x - ?I x r ≤ d - 1"
	    then have "∃ j∈ {1 .. d - 1}. x - ?I x r =  j" by simp
	    then have "∃ j∈ {1 .. d}. x - ?I x r =  j" by auto
	    then obtain  "j" where con: "1≤j ∧ j ≤ d  ∧ x - ?I x r = j" by auto
	    then have xeqr: "x = ?I x r + j" by auto
	    with prems have "?Q (?I x r + j)" by simp
	    with con have qrpj: "∃ j∈ {1 .. d}. ?Q (?I x r + j)" by auto
	    from prems have "∀j∈{1..d}. ¬ ?Q (?I a r + j)" by auto
	    then have "¬ (∃ j∈{1..d}. ?Q (?I x r + j))" 
	      by (simp add: intterm_novar0[OF lininr, where x="x" and y="a"])
	    with qrpj prems have "- x + d + ?I x r ≤ 0" by simp 
	    
	  }
	  ultimately show "- x + d + ?I x r ≤ 0" by blast
	qed
      }
...
next 
  ...
  { assume "∃ i n r. t = Add (Mult (Cst i) (Var n) ) r"
	...
	from prems have "¬ (?Q ((?I a (lin_add(lin_neg r, Cst -1))) + 1))"
	  by auto
	hence "¬ (?Q ((- ?I a r - 1) + 1))" 
	...
	from prems have "¬ (?Q ((?I a (lin_add(r, Cst -1))) + 1))"
	  by auto
	hence "¬ (?Q (?I a r))" 
	  using lin_add_corr lin_neg_corr linr lin_neg_lin
	  by simp
...
next
  ...
  { assume "∃ i n r. t = Add (Mult (Cst i) (Var n) ) r"
	...
	from prems have "¬ (?Q ((?I a (lin_neg r)) + d))"
	  by auto
	hence "¬ (?Q (- ?I a r + d))" 
...
lemma bset_disj_repeat2:
  ...
  shows "∀ x. ¬(∃j∈ {1 .. (divlcm p)}. ∃ b ∈ set (bset p). 
  (qinterp (((I_intterm (a#ats) b) + j)#ats) p))  
  --> (qinterp (x#ats) p) --> (qinterp ((x - (divlcm p))#ats) p)" 
  (is "∀ x. ¬(∃ j∈ {1 .. ?d}. ∃ b∈ ?B. ?P (?I a b + j)) --> ?P x --> ?P (x - ?d)")
proof
  fix x
  have linp: "islinform p" by (rule unified_islinform[OF unifp])
  have dpos: "?d > 0" by (rule divlcm_pos[OF linp])
  have alldvd: "alldivide(?d,p)" by (rule divlcm_corr[OF linp])
    show "¬(∃ j∈ {1 .. ?d}. ∃ b∈ ?B. ?P (?I a b + j)) --> ?P x --> ?P (x - ?d)"
...
lemma cooper_mi_eq: 
  ...
  ((∃ j ∈ {1 .. (divlcm p)}. qinterp (j#ats) (minusinf p)) ∨ 
  (∃ j ∈ {1 .. (divlcm p)}. ∃ b ∈ set (bset p). 
  qinterp (((I_intterm (a#ats) b) + j)#ats) p))"
  (is "(∃ x. ?P x) = ((∃ j∈ {1 .. ?d}. ?MP j) ∨ (∃ j ∈ ?D. ∃ b∈ ?B. ?P (?I a b + j)))")
proof-
  have linp :"islinform p" by (rule unified_islinform[OF unifp])
  have dpos: "?d > 0" by (rule divlcm_pos[OF linp])
  have alldvd: "alldivide(?d,p)" by (rule divlcm_corr[OF linp])
  have eMPimpeP: "(∃j ∈ ?D. ?MP j) --> (∃x. ?P x)"
    by (simp add: minusinf_lemma[OF unifp, where d="?d" and ats="ats"])
  have ePimpeP: "(∃ j ∈ ?D. ∃ b∈ ?B. ?P (?I a b + j)) --> (∃ x. ?P x)"
    by blast
  have bst_rep: "∀ x. ¬ (∃ j ∈ ?D. ∃ b ∈ ?B. ?P (?I a b + j)) --> ?P x --> ?P (x - ?d)"
  ...
  have MPeqP: "∃ z. ∀  x < z. ?P x = ?MP x"
    by (rule minusinf_eq[OF unifp])
  let ?B'= "{?I a b| b. b∈ ?B}"
  from bst_rep have bst_rep2: "∀x. ¬ (∃j∈?D. ∃b∈ ?B'. ?P (b+j)) --> ?P x --> ?P (x - ?d)"
    by auto
  show ?thesis 
...
lemma cooper_pi_eq:
  ...
  qinterp (((I_intterm (a#ats) b) - j)#ats) p))"
  (is "(∃ x. ?P x) = ((∃ j∈ {1 .. ?d}. ?PP (-j)) ∨ (∃ j ∈ ?D. ∃ b∈ ?A. ?P (?I a b - j)))")
proof-
  have unifmp: "isunified (mirror p)" by (rule mirror_unified[OF unifp])
  have th1: 
    "(∃ j∈ {1 .. ?d}. ?PP (-j)) = (∃ j∈ {1..?d}.  qinterp (j # ats) (minusinf (mirror p)))"
  ...
  have "(∃ j ∈ ?D. ∃ b∈ ?A. ?P (?I a b - j)) = 
    (∃ j∈ ?D. ∃ b ∈ set (map lin_neg (bset (mirror p))). ?P (?I a b - j))"
    by (simp only: aset_eq_bset_mirror[OF unifp])
  also have "… = (∃ j∈ ?D. ∃ b ∈ set (bset (mirror p)). ?P (?I a (lin_neg b) - j))"
    by simp
  also have "… = (∃ j∈ ?D. ∃ b ∈ set (bset (mirror p)). ?P (-(?I a b + j)))"
  proof
    assume "∃j∈{1..divlcm p}.
      ∃b∈set (bset (mirror p)). qinterp ((I_intterm (a # ats) (lin_neg b) - j) # ats) p"
    then
    obtain "j" and "b" where 
      pbmj: "j∈ ?D ∧ b∈ set (bset (mirror p)) ∧ ?P (?I a (lin_neg b) - j)" by blast
  ...
  next 
    ...
    from linb pbmj have "?P (?I a (lin_neg b) - j)"  
      by (simp add: lin_neg_corr)
    then show "∃ j∈ ?D. ∃ b ∈ set (bset (mirror p)). ?P (?I a (lin_neg b) - j)"
      using pbmj by auto
  qed
  finally 
  ...
  from bth dth th1
  have "(∃ x. ?P x) = (∃ x. qinterp (x#ats) (mirror p))"
    by (simp add: mirror_ex[OF unifp])
  also have "… = ((∃j∈{1..divlcm (mirror p)}. qinterp (j # ats) (minusinf (mirror p))) ∨
    (∃j∈{1..divlcm (mirror p)}.
    ∃b∈set (bset (mirror p)). qinterp ((I_intterm (a # ats) b + j) # ats) (mirror p)))"
    (is "(∃ x. ?MP x) = ((∃ j∈ ?DM. ?MPM j) ∨ (∃ j ∈ ?DM. ∃ b∈ ?BM. ?MP (?I a b + j)))")
    by (rule cooper_mi_eq[OF unifmp])
  also 
  have "… = ((∃ j∈ ?D. ?PP (-j)) ∨ (∃ j ∈ ?D. ∃ b∈ ?BM. ?MP (?I a b + j)))"
...
consts subst_it:: "intterm ⇒ intterm ⇒ intterm"
primrec
...
proof-
  have "∃ na > 0. islintn(na, a)" by (rule lin_novar0[OF lina nov0a]) 
  then obtain "na" where na: "na > 0 ∧ islintn(na,a)" by blast
  have "∃ nb > 0. islintn(nb, b)" by (rule lin_novar0[OF linb nov0b]) 
...
lemma intterm_subt_novar0:
  ...
  shows "novar0I r"
proof-
  have cnz: "c ≠ 0" by (rule islinintterm_cnz[OF lincnr])
...
lemma decrvars_qfree: "isqfree p ==> isqfree (decrvars p)"
by (induct p rule: isqfree.induct, auto)

lemma novar0_qfree: "novar0 p ==> isqfree p"
by (induct p) auto
...
lemma linearize_novar0: 
  ...
  shows "⋀ t'. linearize t = Some t' ==> novar0I t'"
using nov0t
proof(induct t rule: novar0I.induct)
  case (Neg a)
  let ?la = "linearize a"
  from prems have "∃ a'. ?la = Some a'" by (cases ?la, auto)
...
"psimpl (Divides (Cst d) t) = 
  (case (linearize t) of
  None ⇒ (Divides (Cst d) t)
  | Some c ⇒ (case c of
     Cst i ⇒ (if d dvd i then T else F)
   | _ ⇒  (Divides (Cst d) c)))"
...
proof(induct p rule: psimpl.induct)
  case (1 l r)
  have "(∃ lx. linearize (Sub l r) = Some lx) ∨ (linearize (Sub l r) = None)" by auto
  moreover
  {
...
consts explode_disj :: "(intterm list × QF) ⇒ QF"
recdef explode_disj "measure (λ(is,p). length is)"
"explode_disj ([],p) = F"
"explode_disj (i#is,p) = 
...
lemma explode_disj_disj: 
  ...
  (qinterp (x#xs) (subst_p i p) ∨ (qinterp (x#xs) (explode_disj(is,p))))"
  using qfp
proof-
  let ?pi = "psimpl (subst_p i p)"
  ...
  next
    ...
    have dp_cases: "explode_disj(i#is,p) = 
      ...
      T ⇒ T
      | F ⇒ psimpl (subst_p i p)
      | _ ⇒ Or (psimpl (subst_p i p)) (explode_disj(is,p)))" using notTF
      by (cases "?pi")
...
lemma explode_disj_corr: 
  assumes qfp: "isqfree p"
  shows "(∃ x ∈ set xs. qinterp (a#ats) (subst_p x p)) = 
  (qinterp (a#ats) (explode_disj(xs,p)))" (is "(∃ x ∈ set xs. ?P x) = (?DP a xs )")
  using qfp
  proof (induct xs)
    case Nil show ?case by simp
  next 
    case (Cons y ys)
    have "(∃ x ∈ set (y#ys). ?P x) = (?P y ∨ (∃ x∈ set ys. ?P x))"
      by auto
    also have "… = (?P y ∨ ?DP a ys)" using "Cons.hyps" qfp by auto 
    also have "… = ?DP a (y#ys)" using explode_disj_disj[OF qfp] by auto
    finally show ?case by simp
qed

(* explode_disj preserves independence on Var 0*)
lemma explode_disj_novar0:
  assumes nov0xs: "∀x ∈ set xs. novar0I x"
  and qfp: "isqfree p"
...
proof (induct xs, auto simp add: Let_def)
  ...
  let ?q = "subst_p a p"
  let ?qs = "psimpl ?q"
  have "?qs = T ∨ ?qs = F ∨ (?qs ≠ T ∨ ?qs ≠ F)" by simp
  moreover
...
lemma eval_Or_cases: 
  "qinterp (a#ats) (case f of 
       ...
       | _ ⇒ (case g of 
                     T ⇒ T
                   | F ⇒ f
                   | _ ⇒ Or f g)) = (qinterp (a#ats) f ∨ qinterp (a#ats) g)"
proof-
  let ?result = "
    (case f of 
    T ⇒ T
...
definition
  list_insert :: "'a ⇒ 'a list ⇒ 'a list" where
...
consts list_union :: "('a list × 'a list) ⇒ 'a list"

recdef list_union "measure (λ(xs,ys). length xs)"
"list_union ([], ys) = ys"
"list_union (xs, []) = xs"
...
consts iupto :: "int × int ⇒ int list"
recdef iupto "measure (λ (i,j). nat (j - i +1))"
...
consts all_sums :: "int × intterm list ⇒ intterm list"
recdef all_sums "measure (λ(i,is). length is)"
"all_sums (j,[]) = []"
"all_sums (j,i#is) = (map (λx. lin_add (i,(Cst x))) (iupto(1,j))@(all_sums (j,is)))"
(* all_sums preserves independence on Var 0*)
lemma all_sums_novar0:
  assumes nov0xs: "∀ x∈ set xs. novar0I x"
  and linxs: "∀ x∈ set xs. islinintterm x "
  shows "∀ x∈ set (all_sums (d,xs)). novar0I x"
  using nov0xs linxs
...
next 
  ...
  let ?ys = "map (λx. lin_add (a,(Cst x))) (iupto(1,j))"
  have nov0ys: "∀ y∈ set ?ys. novar0I y"
  proof-
    have linx: "∀ x ∈ set (iupto(1,j)). islinintterm (Cst x)" by simp
    have nov0x: "∀ x ∈ set (iupto(1,j)). novar0I (Cst x)" by simp
    with nov0a lina linx have "∀ x∈ set (iupto(1,j)). novar0I (lin_add (a,Cst x))" 
  ...
  from "2.hyps" linas nov0as have nov0alls: "∀u∈set (all_sums (j, as)). novar0I u" by simp
  from nov0alls nov0ys have 
    cs: "(∀ u∈ set (?ys@ (all_sums (j,as))). novar0I u)"
    by (simp only: sym[OF list_all_iff]) auto
  
  have "all_sums(j,a#as) = ?ys@(all_sums(j,as))"
    by simp
  then 
  have "?case = (∀ x∈ set (?ys@ (all_sums (j,as))). novar0I x)"
    by auto
...
lemma all_sums_ex: 
  "(∃ j∈ {1..d}. ∃ b∈ (set xs). P (lin_add(b,Cst j))) = 
  (∃ x∈ set (all_sums (d,xs)). P x)"
proof(induct d xs rule: all_sums.induct)
  case (1 a) show ?case by simp
next 
  case (2 a y ys)
  have "(∃ x∈ set (map (λx. lin_add (y,(Cst x))) (iupto(1,a))) . P x) = 
    (∃ j∈ set (iupto(1,a)). P (lin_add(y,Cst j)))" 
    by auto
  also have "… = (∃ j∈ {1..a}. P (lin_add(y,Cst j)))" 
    by (simp only : iupto_set)
  finally
  have dsj1:"(∃j∈{1..a}. P (lin_add (y, Cst j))) = (∃x∈set (map (λx. lin_add (y, Cst x)) (iupto (1, a))). P x)" by simp
  
  from prems have "(∃ j∈ {1..a}. ∃ b∈ (set (y#ys)). P (lin_add(b,Cst j))) = 
    ((∃ j∈ {1..a}. P (lin_add(y,Cst j))) ∨ (∃ j∈ {1..a}. ∃ b ∈ set ys. P (lin_add(b,Cst j))))" by auto
  also
  have " … = ((∃ j∈ {1..a}. P (lin_add(y,Cst j))) ∨ (∃ x∈ set (all_sums(a, ys)). P x))" using prems by simp
  also have "… = ((∃x∈set (map (λx. lin_add (y, Cst x)) (iupto (1, a))). P x) ∨ (∃x∈set (all_sums (a, ys)). P x))" using dsj1 by simp
  also have "… = (∃ x∈ (set (map (λx. lin_add (y, Cst x)) (iupto (1, a)))) ∪ (set (all_sums(a, ys))). P x)" by blast
  finally show ?case by simp
qed
...
consts explode_minf :: "(QF × intterm list) ⇒ QF"
...
"explode_minf (q,B) = 
  (let d = divlcm q;
   ...
   in (case dj1 of 
         T ⇒ T
       | F ⇒ explode_disj (all_sums (d,B),q)
        | _ ⇒ (let dj2 = explode_disj (all_sums (d,B),q)
              in ( case dj2 of 
                     T ⇒ T
                   | F ⇒ dj1
                   | _ ⇒ Or dj1 dj2))))"
...
proof-
  ...
  have allsnov0: "∀x∈set (all_sums(?d,B)). novar0I x "
...
lemma explode_minf_corr:
  assumes unifp : "isunified p"
  and bst: "set (bset p) = set B"
  shows "(∃ x . qinterp (x#ats) p) = (qinterp (a#ats) (explode_minf (p,B)))"
  (is "(∃ x. ?P x) = (?EXP a p)")
proof-
  let ?d = "divlcm p"
  let ?pm = "minusinf p"
  ...
  also
  have "… =(∃j∈ set (iupto(1,?d)). qinterp ((I_intterm (a#ats) (Cst j))#ats) (minusinf p))"
    by simp
  also have 
    "… = (∃j∈ set (map Cst (iupto(1,?d))). qinterp ((I_intterm (a#ats) j)#ats) (minusinf p))" by simp
  also have 
    "… = 
    (∃j∈ set (map Cst (iupto(1,?d))). qinterp (a#ats) (subst_p j (minusinf p)))"
  ...
  have allsnov0: "∀x∈set (all_sums(?d,B)). novar0I x "
    by (simp add:all_sums_novar0[OF bstnov0 bstlin] )
  have "(∃ j∈ {1..?d}. ∃ b∈ set B. ?P (I_intterm (a#ats) b + j)) = 
   (∃ j∈ {1..?d}. ∃ b∈ set B. ?P (I_intterm (a#ats) (lin_add(b,Cst j))))"
    using bst by (auto simp add: lin_add_corr bset_lin[OF unifp])
  also have "… = (∃ x ∈ set (all_sums (?d, B)). ?P (I_intterm (a#ats) x))"
    by (simp add: all_sums_ex[where P="λ t. ?P (I_intterm (a#ats) t)"])
  finally 
  have "(∃ j∈ {1..?d}. ∃ b∈ set B. ?P (I_intterm (a#ats) b + j)) = 
    (∃ x ∈ set (all_sums (?d, B)). qinterp (a#ats) (subst_p x p))"
  ...
  also have "… = (qinterp (a#ats) ?dj2)"
    ...
    "(∃ j∈ {1..?d}. ∃ b∈ set B. ?P (I_intterm (a#ats) b + j)) =  
...
proof-
  ...
  also have "…  = (qinterp (a # ats) (explode_minf (p,B)))"
...
(* unify the formula *)
definition unify:: "QF ⇒ (QF × intterm list)" where
  "unify p =
  (let q = unitycoeff p;
       B = list_set(bset q);
       A = list_set (aset q)
...
proof-
  have "length (list_set(bset (unitycoeff p))) ≤ length (list_set (aset (unitycoeff p))) ∨ length (list_set(bset (unitycoeff p))) > length (list_set (aset (unitycoeff p)))" by arith
  moreover
  {
...
proof-
  ...
  {    assume "length ?lb > length ?la"
    ...
    have "set B = set (map lin_neg (map lin_neg (bset (mirror (unitycoeff p)))))"
     ...
     also have "… = set (map (λx. lin_neg (lin_neg x)) (bset (mirror (unitycoeff p))))"
       by auto
...
definition
  cooper:: "QF ⇒ QF option" where
  "cooper p = lift_un (λq. decrvars(explode_minf (unify q))) (linform (nnf p))"

(* cooper eliminates quantifiers *)
lemma cooper_qfree: "(⋀ q q'. [|isqfree q ; cooper q = Some q'|] ==>  isqfree q')"
proof-
  fix "q" "q'"
...
lemma cooper_corr: "(⋀ q q' ats. [|isqfree q ; cooper q = Some q'|] ==>  (qinterp ats (QEx q)) = (qinterp ats q'))"  (is "⋀ q q' ats. [| _ ; _ |] ==> (?P ats (QEx q) = ?P ats q')")
proof-
  fix "q" "q'" "ats"
  assume qfq: "isqfree q"
    and qeq: "cooper q = Some q'"
  ...
  have nnfq: "isnnf (nnf q)" using nnf_isnnf qfq by simp
  then have nnfp: "isnnf p" using linform_nnf[OF nnfq] p_def by auto
  have "∀ ats. ?P ats q = ?P ats (nnf q)" using nnf_corr qfq by auto
...
definition
  pa:: "QF ⇒ QF option" where
  "pa p ≡ lift_un psimpl (qelim(cooper, p))"

lemma psimpl_qfree: "isqfree p ==> isqfree (psimpl p)"
...
apply (case_tac "psimpl p", auto)
apply (case_tac "lift_bin (λx y. lin_add (x, lin_neg y), linearize y,
                   linearize z)", auto)
apply (case_tac "a",auto)
apply (case_tac "lift_bin (λx y. lin_add (x, lin_neg y), linearize ac,
                   linearize ad)", auto)
apply (case_tac "a",auto)
...
theorem pa_corr: 
  "⋀ p'. pa p = Some p' ==> (qinterp ats p = qinterp ats p')"
proof(simp only: pa_def)
  fix "p'"
  assume qep: "lift_un psimpl (qelim(cooper, p)) = Some p'"
 then have "∃ q. qelim (cooper, p) = Some q"
...
then obtain "q" where q_def: "qelim (cooper, p) = Some q" by blast 
  ...
  moreover have cp2: "⋀q q'. [|isqfree q; cooper q = Some q'|] ==> isqfree q'"
